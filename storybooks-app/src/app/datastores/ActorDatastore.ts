...
export class ActorsDatastore implements HasInitialization {
  ...
  private readonly _actorSelectedForPreview = new Subject<AbstractActorDto>();
  public readonly actorSelectedForPreview = this._actorSelectedForPreview.asObservable();
  ...
  private initActorWindowAction() {
    ...
    window["openActor"] = (actorType: string, id: string) => {
      this.zone.run(() => {
        ...
        if (actor) {
          this._actorSelectedForPreview.next(actor);
        }
      });
    };
  }
  ...
  public findActor(id: string) : AbstractActorDto | null {
    return this._actors.value.find(a => a.id === id) ?? null;
  }
}
